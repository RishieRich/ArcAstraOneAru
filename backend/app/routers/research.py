...
from app.research import build_icp, candidates_from_results, feature_enabled, tavily_search
...
def _run(tenant_id, email, kind, params):
    ...
    with get_connection() as conn, conn.cursor() as cur:
        ...
        if kind == "customer":
            ...
            query="Indian businesses buyers " + " ".join(terms)
        else:
            ...
            terms=[product]; query=f"India supplier {product}"
        ...
        try: found=candidates_from_results(tavily_search(query),kind,terms)
        ...
        cur.execute("update research_runs set status='completed' where id=%s",(run_id,)); conn.commit()
    return {"run_id":str(run_id),"status":"completed","candidates_created":len(found)}

@router.post("/customers/run")
def run_customers(tenant_id: UUID = Query(), email: str = Depends(require_dashboard_user)): return _run(str(tenant_id),email,"customer",{})
...
def list_candidates(run_id: UUID, tenant_id: UUID = Query(), status: str | None = Query(default="draft"), email: str = Depends(require_dashboard_user)):
    _enabled()
    with get_connection() as conn, conn.cursor() as cur:
        _access(cur,email,str(tenant_id)); cur.execute("select id,name,location,contact,source_url,retrieved_at,fit_score,fit_reason,status from research_candidates where tenant_id=%s and run_id=%s and (%s is null or status=%s) order by fit_score desc",(str(tenant_id),str(run_id),status,status)); rows=cur.fetchall()
    return [{"id":str(i),"name":n,"location":l,"contact":c,"source_url":u,"retrieved_at":at.isoformat(),"fit_score":s,"fit_reason":r,"status":st} for i,n,l,c,u,at,s,r,st in rows]
